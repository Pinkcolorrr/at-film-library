...
export class PaginationControlService {
   ...
   */
  public firstDocOnPage: QueryDocumentSnapshot<FilmDTO>;

  /**
   * Last document on current page
   */
  public lastDocOnPage: QueryDocumentSnapshot<FilmDTO>;

  /**
   * Array with all first docs
   * Pop last, when switch on prev page
   */
  public firstDocsOnAllPages: QueryDocumentSnapshot<FilmDTO>[] = [];

  /**
   * Get last item in prev page
   */
  public get prevPageStartAt(): QueryDocumentSnapshot<FilmDTO> {
   ...
   */
  public setFirstAndLast(films: DocumentChangeAction<FilmDTO>[]): void {
